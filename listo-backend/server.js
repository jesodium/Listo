...
app.post('/api/register', (req, res) => {
  let { username, wallet_address } = req.body;
  ...
  const sql = 'INSERT INTO users (username, wallet_address) VALUES (?, ?)';
  db.run(sql, [username, wallet_address], function(err) {
    if (err) {
    ...
    res.status(201).json({ success: true, username, wallet_address });
  });
});
...
app.get('/api/lookup/:username', (req, res) => {
  ...
  const sql = 'SELECT wallet_address FROM users WHERE username = ?';
  db.get(sql, [username], (err, row) => {
    ...
    res.json({ wallet_address: row.wallet_address });
  });
});
